...
def RL_processing(data_May,data_June,data_July,Square_root=False):
    ...
    if Square_root==True:
        data_May['Root_length']=np.sqrt(data_May['Root_length']/538)
        data_June['Root_length']=np.sqrt(data_June['Root_length']/538)
        data_July['Root_length']=np.sqrt(data_July['Root_length']/538)
        
    else:
        data_May['Root_length']=data_May['Root_length']/538
        data_June['Root_length']=data_June['Root_length']/538
        data_July['Root_length']=data_July['Root_length']/538
...
def plot_RL(RL_May18,RL_June18,RL_July18,Intervals,n):
    ...
    Mean_RL.plot(figsize=(6,3))
    plt.xticks(np.arange(n), Intervals,rotation=70,size=10) 
    plt.yticks(size=10)
    plt.legend(fontsize=10,loc="upper right")

    #plt.title('Average root profile of all tubes in Bed 1 and Bed 2 for May, June and July 2018')
    plt.ylabel(r'Root length density (cm.cm $^{-3}$)')
